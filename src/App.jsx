...
// ============================================================
// DESIGN SYSTEM â€” #1 updated belts, #8 branding
// ============================================================
const BELT_LEVELS = [
  { id: "white", name: "White Belt", color: "#9CA3AF", bg: "rgba(156,163,175,0.08)", tc: "#fff", level: 1, weeks: 3, workoutsNeeded: 9 },
  ...
  { id: "black", name: "Black Belt", color: "#1F2937", bg: "rgba(31,41,55,0.08)", tc: "#fff", level: 5, weeks: 0, workoutsNeeded: 0 },
];

const C = {
  bg: "#FFFFFF", surface: "#F9FAFB", surfaceHover: "#F3F4F6", border: "#E5E7EB", borderLight: "#D1D5DB",
  text: "#111827", textMuted: "#6B7280", textDim: "#9CA3AF",
  accent: "#F97316", accentLight: "#FB923C", accentGlow: "rgba(249,115,22,0.10)",
  success: "#16A34A", successGlow: "rgba(22,163,74,0.08)", danger: "#EF4444",
  challenge: "#D97706", challengeGlow: "rgba(217,119,6,0.08)",
};
...
function VideoPlayer({ url }) {
  ...
  if (!eu) return (<div style={{ width: "100%", aspectRatio: "16/9", background: C.surface, borderRadius: 16, display: "flex", alignItems: "center", justifyContent: "center", border: `1px dashed ${C.border}` }}><div style={{ textAlign: "center", color: C.textDim }}><div style={{ fontSize: 28, marginBottom: 6 }}>ðŸŽ¬</div><div style={{ fontSize: 13 }}>No video yet</div></div></div>);
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", background: C.bg, borderRadius: 12, border: `1px solid ${C.border}`, marginTop: 10 }}>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        ...
        {rem === 0 && <span style={{ color: C.success, fontWeight: 700, fontSize: 12 }}>âœ“ Done</span>}
        <button onClick={() => { setRem(seconds); setOn(false); clearInterval(ref.current); }} style={{ background: "transparent", border: `1px solid ${C.border}`, borderRadius: 8, padding: "7px 10px", fontSize: 12, color: C.textDim, cursor: "pointer" }}>â†º</button>
      </div>
    </div>
  );
}
...
function Login({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", top: "10%", left: "20%", width: 500, height: 500, borderRadius: "50%", background: "radial-gradient(circle, rgba(249,115,22,0.05), transparent 70%)", filter: "blur(40px)" }} />
      ...
      <div className="fade-in" style={{ width: "100%", maxWidth: 420, position: "relative", zIndex: 1 }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          <div style={{ width: 72, height: 72, borderRadius: 20, background: `linear-gradient(135deg, ${C.accent}, #EA580C)`, display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 36, marginBottom: 16, boxShadow: `0 8px 32px rgba(249,115,22,0.2)` }}>ðŸ€</div>
        ...
        <div style={{ background: C.surface, borderRadius: 20, padding: 32, border: `1px solid ${C.border}` }}>
          {err && <div style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 12, padding: "10px 14px", marginBottom: 16, color: C.danger, fontSize: 13 }}>{err}</div>}
          ...
          <div style={{ marginBottom: 24 }}><label style={{ display: "block", color: C.textMuted, fontSize: 12, marginBottom: 6, fontWeight: 500 }}>Password</label><input type="password" value={pw} onChange={e => setPw(e.target.value)} onKeyDown={e => e.key === "Enter" && go()} style={inp} placeholder="â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢" /></div>
          <button onClick={go} disabled={loading} style={{ width: "100%", background: C.accent, color: "#fff", border: "none", borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 600, cursor: loading ? "default" : "pointer", fontFamily: DISPLAY, opacity: loading ? 0.7 : 1, letterSpacing: 1 }}>{loading ? "Signing in..." : "Sign In"}</button>
        </div>
      </div>
    </div>
  );
...
function Sidebar({ activeTab, setActiveTab, profile, onLogout }) {
  ...
  const tabs = [
    ...
    { id: "workouts", icon: "ðŸ€", label: "Workouts" },
      ...
      <div style={{ width: 240, flexShrink: 0, background: C.surface, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", height: "100vh", position: "sticky", top: 0 }} className="sidebar-desktop">
        <div style={{ padding: "24px 20px 20px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: `linear-gradient(135deg, ${C.accent}, #EA580C)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>ðŸ€</div>
            <span style={{ fontFamily: DISPLAY, fontSize: 17, fontWeight: 700 }}>LEVEL<span style={{ color: C.accent }}>UP</span>BALL</span>
          </div>
          <div style={{ background: "#fff", borderRadius: 14, padding: 14, border: `1px solid ${C.border}` }}>
        ...
        <div style={{ padding: "16px 12px" }}><button onClick={onLogout} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 10, border: `1px solid ${C.border}`, background: "transparent", color: C.textDim, fontSize: 13, cursor: "pointer", fontFamily: FONTS }}><span>â†©</span> Log Out</button></div>
      </div>
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#fff", borderTop: `1px solid ${C.border}`, display: "none", padding: "6px 8px 10px", zIndex: 100 }} className="mobile-nav">
...
// ============================================================
// #6 DASHBOARD â€” progress shows belt journey, #10 next workout clickable
// ============================================================
  ...
  return (
    <div className="fade-in">
      <div style={{ marginBottom: 32 }}>
        ...
        <h1 style={{ fontFamily: DISPLAY, fontSize: 32, fontWeight: 800, letterSpacing: -0.5 }}>{profile.full_name.split(" ")[0]} ðŸ‘Š</h1>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16, marginBottom: 32 }}>
        ...
        <div style={{ background: C.surface, borderRadius: 20, padding: 24, border: `1px solid ${C.border}`, position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            <div style={{ height: 4, background: C.border, borderRadius: 2, marginTop: 14 }}><div style={{ height: "100%", width: `${beltPct}%`, background: C.accent, borderRadius: 2, transition: "width 0.8s" }} /></div>
      ...
      {/* #10 Next workout â€” clickable */}
      {nextWorkout && (
        <div>
          <h2 style={{ fontFamily: DISPLAY, fontSize: 18, fontWeight: 700, marginBottom: 14 }}>Up Next</h2>
          <button onClick={() => onSelectWorkout(nextWorkout)} style={{ width: "100%", background: C.surface, border: `1.5px solid ${C.accent}44`, borderRadius: 18, padding: 20, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", justifyContent: "space-between", fontFamily: FONTS, transition: "all 0.2s" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 8px 24px rgba(0,0,0,0.08)"; }} onMouseLeave={e => { e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = ""; }}>
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <div style={{ width: 56, height: 56, borderRadius: 16, background: C.accent, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: DISPLAY, fontSize: 20, fontWeight: 800, color: "#fff" }}>{nextWorkout.name.replace("Workout ", "")}</div>
              <div><p style={{ fontFamily: DISPLAY, fontSize: 16, fontWeight: 700, color: C.text }}>{nextWorkout.name}</p><p style={{ fontSize: 12, color: C.textDim, marginTop: 4 }}>{(nextWorkout.cats||[]).length} categories Â· {(nextWorkout.cats||[]).reduce((s,c)=>s+(c.exercises||[]).length,0)} exercises</p></div>
            </div>
            <span style={{ color: C.accent, fontSize: 20 }}>â†’</span>
          </button>
        </div>
      )}
...
// ============================================================
// WORKOUTS LIST â€” #3 numbered, #4/#6 show completion status
// ============================================================
function WorkoutsList({ workoutsData, completedIds, completedWorkoutIds, onSelect }) {
  ...
  return (
    <div className="fade-in">
      ...
      {bw.length === 0 ? (
        <div style={{ background: C.surface, borderRadius: 20, padding: 48, border: `1px solid ${C.border}`, textAlign: "center" }}><div style={{ fontSize: 40, marginBottom: 12 }}>ðŸ“‹</div><p style={{ color: C.textMuted }}>No workouts assigned yet</p></div>
      ) : (
          ...
          {bw.map((w, idx) => {
            ...
            return (
              <button key={w.id} onClick={() => onSelect(w)} style={{ background: C.surface, border: wDone ? `1px solid ${C.success}33` : `1px solid ${C.border}`, borderRadius: 16, padding: 18, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", fontFamily: FONTS, transition: "all 0.15s" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <div style={{ width: 48, height: 48, borderRadius: 14, background: wDone ? C.successGlow : C.bg, border: wDone ? `1.5px solid ${C.success}44` : `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: DISPLAY, fontSize: 18, fontWeight: 800, color: wDone ? C.success : C.textDim }}>{wDone ? "âœ“" : idx + 1}</div>
                ...
                <span style={{ color: C.textDim, fontSize: 16 }}>â†’</span>
              </button>
            );
          })}
        </div>
...
// ============================================================
// WORKOUT DETAIL â€” #4 complete button, #5 challenge exercises, #7 supersets
// ============================================================
function WorkoutView({ workout, onBack, completedIds, onToggle, token, profile, completedWorkoutIds, onCompleteWorkout, challengeResults, onSaveChallengeResult }) {
        ...
        <div key={gi} style={{ marginBottom: 8 }}>
          {isSuperset && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, marginLeft: 4 }}>
              <div style={{ width: 18, height: 18, borderRadius: 4, background: `${C.accent}22`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, color: C.accent }}>âš¡</div>
                  ...
                  <div onClick={() => setExpanded(open ? null : ex.id)} style={{ padding: "12px 16px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: 11, color: C.textDim, marginTop: 1 }}>{ex.sets}Ã—{ex.reps} Â· {ex.rest_seconds}s rest</p>
                      </div>
                    </div>
                    <span style={{ color: C.textDim, fontSize: 14, transform: open ? "rotate(180deg)" : "", transition: "transform 0.2s" }}>â–¾</span>
                  </div>
                  {open && (
                    <div style={{ padding: "0 16px 14px" }} className="fade-in">
                      ...
                      {isChallenge && (
                        <div style={{ background: C.challengeGlow, borderRadius: 12, padding: 14, marginTop: 10, border: `1px solid ${C.challenge}33` }}>
                          <p style={{ fontSize: 11, fontWeight: 700, color: C.challenge, letterSpacing: 1, marginBottom: 8 }}>â­ CHALLENGE DRILL</p>
  ...
  return (
    <div className="fade-in">
      <button onClick={onBack} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: "8px 16px", color: C.textMuted, cursor: "pointer", fontFamily: FONTS, fontSize: 13, marginBottom: 20, display: "flex", alignItems: "center", gap: 6 }}>â† Back</button>
      ...
      {!isWorkoutDone ? (
        <button onClick={() => onCompleteWorkout(workout.id)} style={{ width: "100%", background: C.success, color: C.text, border: "none", borderRadius: 14, padding: 16, fontFamily: DISPLAY, fontSize: 16, fontWeight: 700, cursor: "pointer", marginTop: 12, letterSpacing: 0.5 }}>âœ“ Mark Workout Complete</button>
      ) : (
        <div style={{ textAlign: "center", padding: 16, color: C.success, fontFamily: DISPLAY, fontSize: 14, fontWeight: 600 }}>âœ“ Workout Completed!</div>
      )}
    </div>
  );
}
...
// ============================================================
// #11 CHALLENGES â€” with social submissions feed
// ============================================================
function StudentChallenges({ token, profile }) {
  ...
  if (!challenge) return (<div className="fade-in" style={{ textAlign: "center", padding: 48 }}><div style={{ fontSize: 48, marginBottom: 12 }}>ðŸ†</div><p style={{ color: C.textMuted }}>No active challenge right now</p></div>);
  ...
  return (
    <div className="fade-in">
      ...
      {/* Challenge banner */}
      <div style={{ background: C.accent, borderRadius: 20, padding: 28, marginBottom: 20, position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: -40, right: -30, fontSize: 100, opacity: 0.1 }}>ðŸ†</div>
        ...
        <h2 style={{ fontFamily: DISPLAY, fontSize: 24, fontWeight: 800, color: C.text }}>{challenge.title}</h2>
        {deadlineStr && (<div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(0,0,0,0.15)", borderRadius: 8, padding: "5px 14px", marginTop: 12 }}><span style={{ fontSize: 11 }}>â°</span><span style={{ fontSize: 12, fontWeight: 600, color: C.text }}>{isExpired ? "Ended" : `Due ${deadlineStr}`}</span></div>)}
      </div>
      ...
      <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 24, marginTop: 8 }}>
        <h2 style={{ fontFamily: DISPLAY, fontSize: 18, fontWeight: 700, marginBottom: 16 }}>Submissions ðŸ”¥</h2>
        ...
        {!isExpired && submissions.find(s => s.student_id === profile.id) && (
          <div style={{ background: C.successGlow, borderRadius: 12, padding: "12px 16px", marginBottom: 16, border: `1px solid ${C.success}33`, display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ color: C.success, fontWeight: 700 }}>âœ“</span>
        ...
        {submissions.length === 0 ? (
          <div style={{ textAlign: "center", padding: 24, color: C.textDim }}><p>No submissions yet. Be the first! ðŸ€</p></div>
        ) : submissions.map(sub => {
...
// ============================================================
// RESOURCES â€” #2 with video support
// ============================================================
function StudentResources({ token }) {
  ...
  return (
    <div className="fade-in">
      ...
      {articles.length === 0 ? (
        <div style={{ background: C.surface, borderRadius: 20, padding: 48, border: `1px solid ${C.border}`, textAlign: "center" }}><div style={{ fontSize: 36, marginBottom: 12 }}>ðŸ“–</div><p style={{ color: C.textMuted }}>No resources yet</p></div>
      ) : articles.map(a => (
        <div key={a.id} style={{ background: C.surface, borderRadius: 16, border: `1px solid ${C.border}`, marginBottom: 10, overflow: "hidden" }}>
          <div onClick={() => setExpanded(expanded === a.id ? null : a.id)} style={{ padding: "16px 20px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}><div style={{ width: 40, height: 40, borderRadius: 12, background: C.accentGlow, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>ðŸ“–</div><div><p style={{ fontFamily: DISPLAY, fontSize: 14, fontWeight: 600 }}>{a.title}</p><p style={{ fontSize: 11, color: C.textDim, marginTop: 2 }}>{new Date(a.created_at).toLocaleDateString()}</p></div></div>
            <span style={{ color: C.textDim, fontSize: 14, transform: expanded === a.id ? "rotate(180deg)" : "", transition: "transform 0.2s" }}>â–¾</span>
          </div>
...
function LibrarySearch({ token, onAdd, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: C.surface, borderRadius: 16, border: `1px solid ${C.border}`, width: "100%", maxWidth: 600, maxHeight: "80vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ padding: "12px 20px", borderBottom: `1px solid ${C.border}` }}>
          <input
            ...
            style={{ width: "100%", background: "#fff", border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 14px", color: C.text, fontSize: 14, outline: "none" }}
          />
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: "12px 20px" }}>
          {loading ? <p style={{ color: C.textDim, textAlign: "center", padding: 20 }}>Loading library...</p> :
           filtered.length === 0 ? <p style={{ color: C.textDim, textAlign: "center", padding: 20 }}>No exercises found. Add some in the Library tab first!</p> :
           Object.entries(grouped).map(([category, exercises]) => (
            <div key={category} style={{ marginBottom: 20 }}>
              <p style={{ fontFamily: F, fontSize: 11, color: C.accent, letterSpacing: 2, marginBottom: 8 }}>{category.toUpperCase()}</p>
              {exercises.map(ex => (
                <div key={ex.id} style={{ background: "#fff", borderRadius: 10, padding: "12px 14px", marginBottom: 6, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div>
                    <p style={{ fontFamily: F, fontSize: 14, color: C.text, fontWeight: 500 }}>{ex.name}</p>
                    <p style={{ fontSize: 11, color: C.textDim, marginTop: 2 }}>{ex.default_sets}Ã—{ex.default_reps} Â· {ex.default_rest_seconds}s rest</p>
                  </div>
                  <button onClick={() => onAdd(ex)} style={{ background: C.accent, border: "none", borderRadius: 6, padding: "6px 14px", color: "#fff", fontFamily: F, fontSize: 11, letterSpacing: 1, cursor: "pointer", fontWeight: 600, flexShrink: 0 }}>+ ADD</button>
                </div>
              ))}
            </div>
          ))}
...
function AdminSubmissions({ token }) {
  ...
  if (loading) return <p style={{ color: C.textDim, fontSize: 12 }}>Loading submissions...</p>;
  if (subs.length === 0) return <p style={{ color: C.textDim, fontSize: 12 }}>No submissions yet.</p>;
  return subs.map(s => {
    const author = profiles[s.student_id];
    return (
      <div key={s.id} style={{ background: "#fff", borderRadius: 10, border: `1px solid ${C.border}`, marginBottom: 8, padding: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <div><span style={{ fontFamily: F, fontSize: 13, color: C.text, fontWeight: 600 }}>{author?.full_name || "Unknown"}</span><span style={{ fontSize: 11, color: C.textDim, marginLeft: 8 }}>{new Date(s.created_at).toLocaleDateString()}</span></div>
          <button onClick={() => deleteSub(s.id)} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 6, padding: "3px 10px", color: C.danger, fontSize: 10, cursor: "pointer", fontFamily: F }}>DELETE</button>
        </div>
        <div style={{ borderRadius: 10, overflow: "hidden" }}><VideoPlayer url={s.video_url} /></div>
        {s.caption && <p style={{ fontSize: 12, color: C.textMuted, marginTop: 6 }}>{s.caption}</p>}
      </div>
    );
  });
}
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
        <h2 style={{ fontFamily: F, fontSize: 20, color: C.text, fontWeight: 600, letterSpacing: 1 }}>MESSAGES {totalUnread > 0 && <span style={{ background: "#EF4444", color: C.text, fontSize: 11, padding: "2px 8px", borderRadius: 10, marginLeft: 8, fontWeight: 700 }}>{totalUnread} new</span>}</h2>
      </div>

      <div style={{ display: "flex", gap: 14, minHeight: 500 }}>
        {/* Student list */}
        <div style={{ width: 220, flexShrink: 0, background: C.surface, borderRadius: 14, border: `1px solid ${C.border}`, overflow: "hidden" }}>
          <div style={{ padding: "12px 14px", borderBottom: `1px solid ${C.border}` }}>
            <p style={{ fontFamily: F, fontSize: 10, color: C.textDim, letterSpacing: 2 }}>STUDENTS</p>
          </div>
          <div style={{ overflowY: "auto", maxHeight: 450 }}>
            {students.map(s => {
              ...
              return (
                <button key={s.id} onClick={() => selectStudent(s)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", border: "none", borderBottom: `1px solid ${C.border}`, background: isActive ? C.accentGlow : "transparent", cursor: "pointer", textAlign: "left", fontFamily: FONTS }}>
                  <span style={{ fontSize: 13, color: isActive ? C.accent : C.textMuted, fontWeight: unread > 0 ? 700 : 400 }}>{s.full_name}</span>
                  {unread > 0 && <span style={{ background: "#EF4444", color: C.text, fontSize: 9, fontWeight: 700, padding: "2px 6px", borderRadius: 8, minWidth: 18, textAlign: "center" }}>{unread}</span>}
                </button>
              );
            })}
          </div>
        ...
        {/* Chat area */}
        <div style={{ flex: 1, background: C.surface, borderRadius: 14, border: `1px solid ${C.border}`, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          {!selectedStudent ? (
            <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", color: C.textDim }}><div style={{ textAlign: "center" }}><div style={{ fontSize: 32, marginBottom: 8 }}>ðŸ’¬</div><p>Select a student to chat</p></div></div>
          ) : (
            <>
              <div style={{ padding: "12px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontFamily: F, fontSize: 14, color: C.text, fontWeight: 600 }}>{selectedStudent.full_name}</span>
              </div>
              <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
                {messages.length === 0 && <div style={{ textAlign: "center", padding: 24, color: C.textDim }}><p style={{ fontSize: 13 }}>No messages yet</p></div>}
                {messages.map(m => (
                  <div key={m.id} style={{ display: "flex", justifyContent: m.sender_role === "admin" ? "flex-end" : "flex-start", marginBottom: 8 }}>
                    <div style={{ maxWidth: "75%", padding: "10px 14px", borderRadius: m.sender_role === "admin" ? "14px 14px 4px 14px" : "14px 14px 14px 4px", background: m.sender_role === "admin" ? C.accent : "#fff", border: m.sender_role === "admin" ? "none" : `1px solid ${C.border}` }}>
                      <p style={{ fontSize: 13, color: C.text, lineHeight: 1.5 }}>{m.content}</p>
                      <p style={{ fontSize: 10, color: m.sender_role === "admin" ? "rgba(255,255,255,0.6)" : C.textDim, marginTop: 4, textAlign: "right" }}>{new Date(m.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                    </div>
                  </div>
                ))}
                <div ref={bottomRef} />
              ...
              <div style={{ padding: "10px 14px", borderTop: `1px solid ${C.border}`, display: "flex", gap: 8 }}>
                <input value={newMsg} onChange={e => setNewMsg(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} placeholder="Type a reply..." style={{ flex: 1, background: "#fff", border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", color: C.text, fontSize: 13, outline: "none", fontFamily: FONTS }} />
                <button onClick={send} disabled={!newMsg.trim() || sending} style={{ background: C.accent, color: "#fff", border: "none", borderRadius: 10, padding: "10px 18px", fontSize: 12, fontWeight: 600, cursor: newMsg.trim() ? "pointer" : "default", fontFamily: F, opacity: newMsg.trim() ? 1 : 0.5 }}>Send</button>
              </div>
            </>
          )}
        </div>
...
function Admin({ token }) {
  ...
  const inp = { width: "100%", background: "#fff", border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 14px", color: C.text, fontSize: 14, outline: "none" };
  ...
  return (
    <div style={{ maxWidth: 960, margin: "0 auto", padding: 24 }}>
      {msg && <div style={{ position: "fixed", top: 70, right: 20, background: msg.startsWith("Error") ? C.danger : C.success, color: "#fff", padding: "10px 20px", borderRadius: 8, fontFamily: F, fontSize: 13, letterSpacing: 1, zIndex: 200 }}>{msg}</div>}
      {saving && <div style={{ position: "fixed", top: 70, left: "50%", transform: "translateX(-50%)", background: C.accent, color: "#fff", padding: "6px 16px", borderRadius: 6, fontFamily: F, fontSize: 11, letterSpacing: 1, zIndex: 200 }}>SAVING...</div>}
      ...
      <div style={{ display: "flex", gap: 0, marginBottom: 24, borderBottom: `1px solid ${C.border}` }}>
        {["workouts", "library", "articles", "challenges", "students", "messages"].map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ background: "none", border: "none", borderBottom: tab === t ? `2px solid ${C.accent}` : "2px solid transparent", padding: "12px 20px", color: tab === t ? C.accent : C.textDim, fontFamily: F, fontSize: 13, letterSpacing: 2, cursor: "pointer", fontWeight: 600 }}>{t.toUpperCase()}</button>
        ))}
      </div>
      ...
      {tab === "workouts" && <>
        <div style={{ display: "flex", gap: 6, marginBottom: 20, flexWrap: "wrap" }}>
          {BELT_LEVELS.map(b => (
            <button key={b.id} onClick={() => { setBelt(b.id); setEditing(null); }} style={{ background: belt === b.id ? b.color : C.surface, color: belt === b.id ? b.tc : C.textMuted, border: belt === b.id ? "none" : `1px solid ${C.border}`, borderRadius: 8, padding: "7px 16px", fontFamily: F, fontSize: 12, letterSpacing: 1, cursor: "pointer", fontWeight: 600 }}>{b.name.toUpperCase()}</button>
          ))}
        </div>
        {loading ? <div style={{ textAlign: "center", padding: 40, color: C.textMuted }}>Loading workouts...</div> : !editing ? <>
          {workouts.map(w => {
            const ec = (w.cats || []).reduce((s, c) => s + (c.exercises || []).length, 0);
            return (
              <div key={w.id} style={{ background: C.surface, borderRadius: 12, padding: 18, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                <div>
                  <p style={{ fontFamily: F, fontSize: 16, color: C.text, fontWeight: 600 }}>{w.name}</p>
                  <p style={{ color: C.textDim, fontSize: 12, marginTop: 2 }}>{(w.cats || []).length} categories Â· {ec} exercises</p>
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => setEditing(w)} style={{ background: C.accent, border: "none", borderRadius: 8, padding: "7px 16px", color: "#fff", fontFamily: F, fontSize: 11, letterSpacing: 1, cursor: "pointer", fontWeight: 600 }}>EDIT</button>
                  ...
                  <button onClick={() => deleteWorkout(w.id)} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 8, padding: "7px 12px", color: C.danger, fontFamily: F, fontSize: 11, letterSpacing: 1, cursor: "pointer" }}>DELETE</button>
                </div>
              </div>
            );
          })}
          <button onClick={addWorkout} disabled={saving} style={{ background: C.surface, border: `2px dashed ${C.border}`, borderRadius: 12, padding: 18, width: "100%", color: C.accent, fontFamily: F, fontSize: 13, letterSpacing: 1, cursor: "pointer", fontWeight: 600, marginTop: 6 }}>+ ADD WORKOUT</button>
        </> : <>
          <button onClick={() => { setEditing(null); loadWorkouts(); }} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: "7px 14px", color: C.text, fontFamily: F, fontSize: 12, letterSpacing: 1, cursor: "pointer", marginBottom: 18 }}>â† BACK TO LIST</button>
          <div style={{ marginBottom: 20 }}>
            <label style={{ display: "block", color: C.textDim, fontSize: 10, fontFamily: F, letterSpacing: 1, marginBottom: 5 }}>WORKOUT NAME</label>
          ...
          {(editing.cats || []).map(cat => (
            <div key={cat.id} style={{ background: C.surface, borderRadius: 14, padding: 18, border: `1px solid ${C.border}`, marginBottom: 14 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                ...
                <button onClick={() => deleteCategory(cat.id, editing.id)} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 6, padding: "5px 10px", color: C.danger, fontSize: 10, fontFamily: F, cursor: "pointer", letterSpacing: 1 }}>REMOVE</button>
              </div>
              {(cat.exercises || []).map(ex => (
                <div key={ex.id} style={{ background: "#fff", borderRadius: 10, padding: 14, marginBottom: 8, border: `1px solid ${C.border}` }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                    ...
                    <button onClick={() => deleteExercise(ex.id, editing.id)} style={{ background: "none", border: "none", color: C.danger, cursor: "pointer", fontSize: 15, marginLeft: 10, flexShrink: 0 }}>âœ•</button>
                  </div>
                  <div style={{ marginBottom: 8 }}>
                    <label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>VIDEO URL (YouTube / Vimeo)</label>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 8 }}>
                    {[["SETS", "sets"], ["REPS", "reps"], ["REST (sec)", "rest_seconds"]].map(([l, k]) => (
                      <div key={k}>
                        <label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>{l}</label>
                  ...
                  <div>
                    <div style={{ display: "flex", gap: 12, marginBottom: 8 }}>
                      <label style={{ display: "flex", alignItems: "center", gap: 6, color: C.textMuted, fontSize: 11, cursor: "pointer" }}><input type="checkbox" defaultChecked={ex.is_challenge} onChange={e => saveExercise(ex.id, "is_challenge", e.target.checked)} style={{ width: 16, height: 16 }} /> Challenge Exercise</label>
                      <div style={{ flex: 1 }}><label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>SUPERSET GROUP</label><input defaultValue={ex.superset_group || ""} onBlur={e => saveExercise(ex.id, "superset_group", e.target.value)} style={{ width: "100%", background: "#fff", border: `1px solid ${C.border}`, borderRadius: 8, padding: "6px 10px", color: C.text, fontSize: 12, outline: "none" }} placeholder="e.g. A" /></div>
                    </div>
                    <label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>INSTRUCTIONS</label>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setShowLibSearch({ catId: cat.id, workoutId: editing.id })} style={{ background: C.accent, border: "none", borderRadius: 8, padding: 9, flex: 1, color: "#fff", fontFamily: F, fontSize: 11, letterSpacing: 1, cursor: "pointer", fontWeight: 600 }}>ðŸ“š ADD FROM LIBRARY</button>
                <button onClick={() => addExercise(cat.id, editing.id)} disabled={saving} style={{ background: "none", border: `1px dashed ${C.border}`, borderRadius: 8, padding: 9, flex: 1, color: C.textDim, fontFamily: F, fontSize: 11, letterSpacing: 1, cursor: "pointer" }}>+ ADD CUSTOM</button>
              </div>
            </div>
          ))}
          <button onClick={() => addCategory(editing.id)} disabled={saving} style={{ background: C.surface, border: `2px dashed ${C.border}`, borderRadius: 12, padding: 14, width: "100%", color: C.accent, fontFamily: F, fontSize: 12, letterSpacing: 1, cursor: "pointer", fontWeight: 600 }}>+ ADD CATEGORY</button>
        </>}
      </>}
      ...
      {tab === "library" && <>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, flexWrap: "wrap", gap: 10 }}>
          <h2 style={{ fontFamily: F, fontSize: 20, color: C.text, fontWeight: 600, letterSpacing: 1 }}>EXERCISE LIBRARY</h2>
          <button onClick={startNewLib} style={{ background: C.accent, border: "none", borderRadius: 8, padding: "9px 18px", color: "#fff", fontFamily: F, fontSize: 12, letterSpacing: 1, cursor: "pointer", fontWeight: 600 }}>+ NEW EXERCISE</button>
        </div>
        ...
        {(libNew || libEditing) && (
          <div style={{ background: C.surface, borderRadius: 14, padding: 20, border: `1px solid ${C.border}`, marginBottom: 20 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
              <div>
                <label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>NAME *</label>
                <input value={libForm.name} onChange={e => setLibForm({ ...libForm, name: e.target.value })} style={inp} placeholder="e.g. Crossover Dribble" />
              </div>
              <div>
                <label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>CATEGORY *</label>
            ...
            <div style={{ marginBottom: 10 }}>
              <label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>VIDEO URL</label>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 10 }}>
              <div>
                <label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>DEFAULT SETS</label>
              ...
              <div>
                <label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>DEFAULT REPS</label>
              ...
              <div>
                <label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>DEFAULT REST (sec)</label>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              <button onClick={saveLibItem} disabled={saving} style={{ background: C.success, border: "none", borderRadius: 8, padding: "9px 18px", color: "#fff", fontFamily: F, fontSize: 11, letterSpacing: 1, cursor: "pointer", fontWeight: 600 }}>SAVE</button>
              <button onClick={() => { setLibEditing(null); setLibNew(false); }} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 8, padding: "9px 18px", color: C.textMuted, fontFamily: F, fontSize: 11, letterSpacing: 1, cursor: "pointer" }}>CANCEL</button>
            </div>
          </div>
        )}

        {/* Library List */}
        {libLoading ? <p style={{ color: C.textDim, padding: 20, textAlign: "center" }}>Loading...</p> :
         Object.keys(libGrouped).length === 0 ? (
          <div style={{ textAlign: "center", padding: 40, color: C.textDim }}>
            <p style={{ fontSize: 36, marginBottom: 10 }}>ðŸ“š</p>
          ...
          <div key={category} style={{ marginBottom: 24 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
              ...
              <p style={{ fontFamily: F, fontSize: 14, color: C.accent, letterSpacing: 2, fontWeight: 600 }}>{category.toUpperCase()}</p>
              <span style={{ fontSize: 11, color: C.textDim }}>{exercises.length}</span>
            </div>
            {exercises.map(ex => (
              <div key={ex.id} style={{ background: C.surface, borderRadius: 10, padding: "14px 16px", marginBottom: 6, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
                  <p style={{ fontFamily: F, fontSize: 14, color: C.text, fontWeight: 500 }}>{ex.name}</p>
                  <p style={{ fontSize: 11, color: C.textDim, marginTop: 2 }}>{ex.default_sets}Ã—{ex.default_reps} Â· {ex.default_rest_seconds}s rest {ex.video_url ? "Â· ðŸŽ¬" : ""}</p>
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => startEditLib(ex)} style={{ background: C.surfaceHover, border: "none", borderRadius: 6, padding: "5px 12px", color: C.text, fontFamily: F, fontSize: 10, letterSpacing: 1, cursor: "pointer" }}>EDIT</button>
                  <button onClick={() => deleteLibItem(ex.id)} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 6, padding: "5px 12px", color: C.danger, fontFamily: F, fontSize: 10, letterSpacing: 1, cursor: "pointer" }}>DELETE</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ===== ARTICLES ===== */}
      {tab === "articles" && <>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}><h2 style={{ fontFamily: F, fontSize: 20, color: "#fff", fontWeight: 600, letterSpacing: 1 }}>RESOURCES / ARTICLES</h2><button onClick={startNewArt} style={{ background: C.accent, border: "none", borderRadius: 8, padding: "9px 18px", color: C.text, fontFamily: F, fontSize: 12, letterSpacing: 1, cursor: "pointer", fontWeight: 600 }}>+ NEW ARTICLE</button></div>
        {(artNew||artEditing) && (
          <div style={{ background: C.surface, borderRadius: 14, padding: 20, border: `1px solid ${C.border}`, marginBottom: 20 }}>
            <h3 style={{ fontFamily: F, fontSize: 14, color: C.text, marginBottom: 14, letterSpacing: 1 }}>{artEditing ? "EDIT" : "NEW"} ARTICLE</h3>
            <div style={{ marginBottom: 10 }}><label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>TITLE *</label><input value={artForm.title} onChange={e=>setArtForm({...artForm,title:e.target.value})} style={inp} placeholder="Article title..." /></div>
            <div style={{ marginBottom: 10 }}><label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>CONTENT</label><textarea value={artForm.content} onChange={e=>setArtForm({...artForm,content:e.target.value})} style={{ ...inp, minHeight: 200, resize: "vertical" }} placeholder="Write your article here..." /></div>
            <div style={{ marginBottom: 10 }}><label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>VIDEO URL (optional)</label><input value={artForm.video_url} onChange={e=>setArtForm({...artForm,video_url:e.target.value})} style={inp} placeholder="https://youtube.com/watch?v=..." /></div>
            <div style={{ marginBottom: 12, display: "flex", alignItems: "center", gap: 10 }}><label style={{ color: C.textDim, fontSize: 11, fontFamily: F, letterSpacing: 1 }}>PUBLISHED</label><input type="checkbox" checked={artForm.published} onChange={e=>setArtForm({...artForm,published:e.target.checked})} style={{ width: 18, height: 18, cursor: "pointer" }} /></div>
            <div style={{ display: "flex", gap: 6 }}><button onClick={saveArticle} disabled={saving} style={{ background: C.success, border: "none", borderRadius: 8, padding: "9px 18px", color: "#fff", fontFamily: F, fontSize: 11, letterSpacing: 1, cursor: "pointer", fontWeight: 600 }}>SAVE</button><button onClick={()=>{setArtEditing(null);setArtNew(false);}} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 8, padding: "9px 18px", color: "#fff"Muted, fontFamily: F, fontSize: 11, letterSpacing: 1, cursor: "pointer" }}>CANCEL</button></div>
          </div>
        )}
        {articles.length===0 ? (<div style={{ textAlign: "center", padding: 40, color: C.textDim }}><p style={{ fontSize: 36, marginBottom: 10 }}>ðŸ“–</p><p>No articles yet.</p></div>) : articles.map(a=>(<div key={a.id} style={{ background: C.surface, borderRadius: 12, padding: 18, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}><div><p style={{ fontFamily: F, fontSize: 15, color: C.text, fontWeight: 600 }}>{a.title}</p><p style={{ fontSize: 11, color: C.textDim, marginTop: 2 }}>{a.published ? "Published" : "Draft"} Â· {new Date(a.created_at).toLocaleDateString()}</p></div><div style={{ display: "flex", gap: 6 }}><button onClick={()=>startEditArt(a)} style={{ background: C.surfaceHover, border: "none", borderRadius: 6, padding: "5px 12px", color: C.text, fontFamily: F, fontSize: 10, letterSpacing: 1, cursor: "pointer" }}>EDIT</button><button onClick={()=>deleteArticle(a.id)} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 6, padding: "5px 12px", color: C.danger, fontFamily: F, fontSize: 10, letterSpacing: 1, cursor: "pointer" }}>DEL</button></div></div>))}
      </>}
      ...
      {tab === "challenges" && <>
        ...
        {(chalNew||chalEditing) && (
          <div style={{ background: C.surface, borderRadius: 14, padding: 20, border: `1px solid ${C.border}`, marginBottom: 20 }}>
            <h3 style={{ fontFamily: F, fontSize: 14, color: C.text, marginBottom: 14, letterSpacing: 1 }}>{chalEditing ? "EDIT" : "NEW"} CHALLENGE</h3>
            <div style={{ marginBottom: 10 }}><label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>TITLE *</label><input value={chalForm.title} onChange={e=>setChalForm({...chalForm,title:e.target.value})} style={inp} placeholder="e.g. 50 Made Free Throws" /></div>
            <div style={{ marginBottom: 10 }}><label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>DESCRIPTION</label><textarea value={chalForm.description} onChange={e=>setChalForm({...chalForm,description:e.target.value})} style={{ ...inp, minHeight: 100, resize: "vertical" }} placeholder="Explain the challenge..." /></div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}><div><label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>DEMO VIDEO URL</label><input value={chalForm.video_url} onChange={e=>setChalForm({...chalForm,video_url:e.target.value})} style={inp} placeholder="https://youtube.com/..." /></div><div><label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>DEADLINE</label><input type="date" value={chalForm.deadline} onChange={e=>setChalForm({...chalForm,deadline:e.target.value})} style={inp} /></div></div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}><div><label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>SUBMISSION EMAIL</label><input value={chalForm.submission_email} onChange={e=>setChalForm({...chalForm,submission_email:e.target.value})} style={inp} /></div><div style={{ display: "flex", alignItems: "center", gap: 10, paddingTop: 18 }}><label style={{ color: C.textDim, fontSize: 11, fontFamily: F, letterSpacing: 1 }}>ACTIVE</label><input type="checkbox" checked={chalForm.active} onChange={e=>setChalForm({...chalForm,active:e.target.checked})} style={{ width: 18, height: 18, cursor: "pointer" }} /><span style={{ fontSize: 10, color: C.accent }}>{chalForm.active ? "Students see this" : "Hidden"}</span></div></div>
        ...
        <div style={{ marginTop: 24, marginBottom: 24 }}>
          <h3 style={{ fontFamily: F, fontSize: 14, color: C.text, letterSpacing: 1, marginBottom: 12 }}>STUDENT SUBMISSIONS</h3>
          <AdminSubmissions token={token} />
        </div>
        {challenges.length===0 ? (<div style={{ textAlign: "center", padding: 40, color: "#fff"Dim }}><p style={{ fontSize: 36, marginBottom: 10 }}>ðŸ†</p><p>No challenges yet.</p></div>) : challenges.map(c=>(<div key={c.id} style={{ background: C.surface, borderRadius: 12, padding: 18, border: c.active ? `2px solid ${C.accent}` : `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}><div><div style={{ display: "flex", alignItems: "center", gap: 8 }}><p style={{ fontFamily: F, fontSize: 15, color: C.text, fontWeight: 600 }}>{c.title}</p>{c.active && <span style={{ background: C.accent, color: "#fff", fontFamily: F, fontSize: 9, padding: "2px 8px", borderRadius: 4, letterSpacing: 1 }}>ACTIVE</span>}</div><p style={{ fontSize: 11, color: C.textDim, marginTop: 2 }}>{c.deadline ? "Due " + new Date(c.deadline+"T12:00:00").toLocaleDateString() : "No deadline"}</p></div><div style={{ display: "flex", gap: 6 }}><button onClick={()=>startEditChal(c)} style={{ background: C.surfaceHover, border: "none", borderRadius: 6, padding: "5px 12px", color: C.text, fontFamily: F, fontSize: 10, letterSpacing: 1, cursor: "pointer" }}>EDIT</button><button onClick={()=>deleteChallenge(c.id)} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 6, padding: "5px 12px", color: C.danger, fontFamily: F, fontSize: 10, letterSpacing: 1, cursor: "pointer" }}>DEL</button></div></div>))}
      </>}
      ...
      {tab === "students" && <>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          <h2 style={{ fontFamily: F, fontSize: 20, color: C.text, fontWeight: 600, letterSpacing: 1 }}>MANAGE STUDENTS</h2>
          <button onClick={() => setShowAdd(!showAdd)} style={{ background: C.accent, border: "none", borderRadius: 8, padding: "9px 18px", color: "#fff", fontFamily: F, fontSize: 12, letterSpacing: 1, cursor: "pointer", fontWeight: 600 }}>+ ADD STUDENT</button>
        </div>
        {showAdd && (
          <div style={{ background: C.surface, borderRadius: 14, padding: 18, border: `1px solid ${C.border}`, marginBottom: 18 }}>
            <h3 style={{ fontFamily: F, fontSize: 14, color: C.text, marginBottom: 14, letterSpacing: 1 }}>NEW STUDENT</h3>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
              {[["NAME", "name", "Player name"], ["EMAIL", "email", "email@example.com"], ["PASSWORD", "password", "Temp password"]].map(([l, k, ph]) => (
                <div key={k}><label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>{l}</label><input value={ns[k]} onChange={e => setNs({ ...ns, [k]: e.target.value })} style={inp} placeholder={ph} /></div>
              ))}
              <div><label style={{ display: "block", color: C.textDim, fontSize: 9, fontFamily: F, letterSpacing: 1, marginBottom: 3 }}>STARTING BELT</label><select value={ns.beltId} onChange={e => setNs({ ...ns, beltId: e.target.value })} style={{ ...inp, cursor: "pointer" }}>{BELT_LEVELS.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select></div>
            </div>
            <div style={{ display: "flex", gap: 6 }}>
              <button onClick={addStudent} disabled={saving} style={{ background: C.success, border: "none", borderRadius: 8, padding: "9px 18px", color: "#fff", fontFamily: F, fontSize: 11, letterSpacing: 1, cursor: "pointer", fontWeight: 600 }}>SAVE</button>
              <button onClick={() => setShowAdd(false)} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 8, padding: "9px 18px", color: C.textMuted, fontFamily: F, fontSize: 11, letterSpacing: 1, cursor: "pointer" }}>CANCEL</button>
            </div>
          </div>
        )}
        {students.map(s => {
          ...
          return (
            <div key={s.id} style={{ background: C.surface, borderRadius: 12, padding: 18, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8, flexWrap: "wrap", gap: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: `${sb.color}22`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, border: `2px solid ${sb.color}` }}>ðŸ€</div>
                <div><p style={{ fontFamily: F, fontSize: 15, color: C.text, fontWeight: 600 }}>{s.full_name}</p><p style={{ fontSize: 11, color: C.textDim }}>{s.email}</p></div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                {s.belt_id !== "black" && <button onClick={() => promoteStudent(s)} disabled={saving} style={{ background: C.success, border: "none", borderRadius: 8, padding: "5px 12px", color: "#fff", fontFamily: F, fontSize: 10, letterSpacing: 1, cursor: "pointer", fontWeight: 600 }}>PROMOTE â†‘</button>}
                {s.belt_id !== "white" && <button onClick={() => demoteStudent(s)} disabled={saving} style={{ background: "#F59E0B", border: "none", borderRadius: 8, padding: "5px 12px", color: "#fff", fontFamily: F, fontSize: 10, letterSpacing: 1, cursor: "pointer", fontWeight: 600 }}>DEMOTE â†“</button>}
                <button onClick={() => deleteStudent(s.id)} disabled={saving} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 8, padding: "5px 12px", color: C.danger, fontFamily: F, fontSize: 10, letterSpacing: 1, cursor: "pointer" }}>REMOVE</button>
              </div>
            </div>
          );
        })}
        {students.length === 0 && <div style={{ textAlign: "center", padding: 36, color: C.textDim }}><p style={{ fontSize: 28, marginBottom: 8 }}>ðŸ‘¥</p><p>No students yet.</p></div>}
      </>}
    </div>
  );
}
  ...
  return (
    <div className="fade-in" style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 100px)" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", background: C.surface, borderRadius: 16, border: `1px solid ${C.border}`, padding: 16, marginBottom: 14 }}>
        {messages.length === 0 && <div style={{ textAlign: "center", padding: 32, color: C.textDim }}><div style={{ fontSize: 32, marginBottom: 8 }}>ðŸ’¬</div><p>No messages yet. Say hi to your coach!</p></div>}
        {messages.map(m => (
          <div key={m.id} style={{ display: "flex", justifyContent: m.sender_role === "student" ? "flex-end" : "flex-start", marginBottom: 8 }}>
            <div style={{ maxWidth: "75%", padding: "10px 14px", borderRadius: m.sender_role === "student" ? "14px 14px 4px 14px" : "14px 14px 14px 4px", background: m.sender_role === "student" ? C.accent : C.surface, border: m.sender_role === "student" ? "none" : `1px solid ${C.border}`, color: m.sender_role === "student" ? "#fff" : C.text }}>
...
function StudentLayout({ profile, token, onLogout }) {
  ...
  if (!loaded) return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ fontSize: 40, animation: "pulse 1.5s infinite" }}>ðŸ€</div></div>;
...
export default function LevelUpBallApp() {
  ...
  if (loading) return <><style>{GLOBAL_CSS}</style><div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: C.bg }}><div style={{ fontSize: 48, animation: "pulse 1.5s infinite" }}>ðŸ€</div></div></>;
